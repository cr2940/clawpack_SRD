...
/*frompyobj*/
  ...
  /* Processing variable num_waves */
    f2py_success = int_from_pyobj(&num_waves,num_waves_capi,"traffic_1D.rp1() 2nd argument (num_waves) can't be converted to int");
  if (f2py_success) {
  /* Processing variable num_ghost */
    f2py_success = int_from_pyobj(&num_ghost,num_ghost_capi,"traffic_1D.rp1() 3rd argument (num_ghost) can't be converted to int");
  if (f2py_success) {
  /* Processing variable num_cells */
    f2py_success = int_from_pyobj(&num_cells,num_cells_capi,"traffic_1D.rp1() 4th argument (num_cells) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_auxl_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting 7th argument `auxl' of traffic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_s_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting hidden `s' of traffic_1D.rp1 to C/Fortran array" );
  } else {
    s = (double *)(PyArray_DATA(capi_s_tmp));

  /* Processing variable num_eqn */
  if (num_eqn_capi == Py_None) num_eqn = shape(ql,0); else
    f2py_success = int_from_pyobj(&num_eqn,num_eqn_capi,"traffic_1D.rp1() 1st keyword (num_eqn) can't be converted to int");
  if (f2py_success) {
  ...
  if (num_aux_capi == Py_None) num_aux = shape(auxl,0); else
    f2py_success = int_from_pyobj(&num_aux,num_aux_capi,"traffic_1D.rp1() 2nd keyword (num_aux) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_auxr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting 8th argument `auxr' of traffic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_wave_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting hidden `wave' of traffic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_amdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting hidden `amdq' of traffic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_apdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(traffic_1D_error,"failed in converting hidden `apdq' of traffic_1D.rp1 to C/Fortran array" );
  } else {
...
"COMMON blocks:\n""  /cparam/ umax\n"".");
   ...
   */
  PyDict_SetItemString(d, "_traffic_1D_error", traffic_1D_error);
  Py_DECREF(traffic_1D_error);
