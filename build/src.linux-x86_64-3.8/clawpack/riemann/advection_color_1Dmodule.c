/* File: advection_color_1Dmodule.c
...
static PyObject *advection_color_1D_error;
static PyObject *advection_color_1D_module;
...
#define CHECKSCALAR(check,tcheck,name,show,var)\
    if (!(check)) {\
        ...
        PyErr_SetString(advection_color_1D_error,errstring);\
        /*goto capi_fail;*/\
    } else 
#ifdef DEBUGCFUNCS
...
static int int_from_pyobj(int* v,PyObject *obj,const char *errmess) {
    ...
    }
    {
        ...
        if (err==NULL) err = advection_color_1D_error;
...
/************************************ rp1 ************************************/
static char doc_f2py_rout_advection_color_1D_rp1[] = "\
...
/* extern void F_FUNC(rp1,RP1)(int*,int*,int*,int*,int*,int*,double*,double*,double*,double*,double*,double*,double*,double*); */
static PyObject *f2py_rout_advection_color_1D_rp1(const PyObject *capi_self,
                           PyObject *capi_args,
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"maxmx","mbc","mx","ql","qr","auxl","auxr","wave","s","amdq","apdq","meqn","mwaves","maux",NULL};
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOOOOOOO|OOO:advection_color_1D.rp1",\
    capi_kwlist,&maxmx_capi,&mbc_capi,&mx_capi,&ql_capi,&qr_capi,&auxl_capi,&auxr_capi,&wave_capi,&s_capi,&amdq_capi,&apdq_capi,&meqn_capi,&mwaves_capi,&maux_capi))
    return NULL;
/*frompyobj*/
  /* Processing variable maxmx */
    f2py_success = int_from_pyobj(&maxmx,maxmx_capi,"advection_color_1D.rp1() 1st argument (maxmx) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mbc */
    f2py_success = int_from_pyobj(&mbc,mbc_capi,"advection_color_1D.rp1() 2nd argument (mbc) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mx */
    f2py_success = int_from_pyobj(&mx,mx_capi,"advection_color_1D.rp1() 3rd argument (mx) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_ql_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 4th argument `ql' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_auxl_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 6th argument `auxl' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
    auxl = (double *)(PyArray_DATA(capi_auxl_tmp));

  /* Processing variable meqn */
  if (meqn_capi == Py_None) meqn = shape(ql,0); else
    f2py_success = int_from_pyobj(&meqn,meqn_capi,"advection_color_1D.rp1() 1st keyword (meqn) can't be converted to int");
  if (f2py_success) {
  ...
  if (maux_capi == Py_None) maux = shape(auxl,0); else
    f2py_success = int_from_pyobj(&maux,maux_capi,"advection_color_1D.rp1() 3rd keyword (maux) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_qr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 5th argument `qr' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_auxr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 7th argument `auxr' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_wave_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 8th argument `wave' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_amdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 10th argument `amdq' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_apdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 11st argument `apdq' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
    apdq = (double *)(PyArray_DATA(capi_apdq_tmp));

  /* Processing variable mwaves */
  if (mwaves_capi == Py_None) mwaves = shape(wave,1); else
    f2py_success = int_from_pyobj(&mwaves,mwaves_capi,"advection_color_1D.rp1() 2nd keyword (mwaves) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_s_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(advection_color_1D_error,"failed in converting 9th argument `s' of advection_color_1D.rp1 to C/Fortran array" );
  } else {
...
static FortranDataDef f2py_comrp_def[] = {
  {"u",0,{{-1}},NPY_DOUBLE},
  {NULL}
};
static void f2py_setup_comrp(char *u) {
  int i_f2py=0;
  f2py_comrp_def[i_f2py++].data = u;
}
extern void F_FUNC(f2pyinitcomrp,F2PYINITCOMRP)(void(*)(char*));
static void f2py_init_comrp(void) {
  F_FUNC(f2pyinitcomrp,F2PYINITCOMRP)(f2py_setup_comrp);
}
...
static FortranDataDef f2py_routine_defs[] = {
  {"rp1",-1,{{-1}},0,(char *)F_FUNC(rp1,RP1),(f2py_init_func)f2py_rout_advection_color_1D_rp1,doc_f2py_rout_advection_color_1D_rp1},
...
#if PY_VERSION_HEX >= 0x03000000
#define RETVAL m
PyMODINIT_FUNC PyInit_advection_color_1D(void) {
#else
#define RETVAL
PyMODINIT_FUNC initadvection_color_1D(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
  m = advection_color_1D_module = PyModule_Create(&moduledef);
#else
  m = advection_color_1D_module = Py_InitModule("advection_color_1D", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module advection_color_1D (failed to import numpy)"); return RETVAL;}
...
#endif
    "This module 'advection_color_1D' is auto-generated with f2py (version:2).\nFunctions:\n"
"  rp1(maxmx,mbc,mx,ql,qr,auxl,auxr,wave,s,amdq,apdq,meqn=shape(ql,0),mwaves=shape(wave,1),maux=shape(auxl,0))\n"
"COMMON blocks:\n""  /comrp/ u\n"".");
  PyDict_SetItemString(d, "__doc__", s);
  Py_DECREF(s);
  advection_color_1D_error = PyErr_NewException ("advection_color_1D.error", NULL, NULL);
  /*
   * Store the error object inside the dict, so that it could get deallocated.
   * (in practice, this is a module, so it likely will not and cannot.)
   */
  PyDict_SetItemString(d, "_advection_color_1D_error", advection_color_1D_error);
  Py_DECREF(advection_color_1D_error);
...
/*eof initf90modhooks*/
  ...
  tmp = PyFortranObject_New(f2py_comrp_def,f2py_init_comrp);
  F2PyDict_SetItemString(d, "comrp", tmp);
  Py_DECREF(tmp);
/*eof initcommonhooks*/
...
#ifdef F2PY_REPORT_ATEXIT
  if (! PyErr_Occurred())
    on_exit(f2py_report_on_exit,(void*)"advection_color_1D");
#endif
  return RETVAL;
}
#ifdef __cplusplus
