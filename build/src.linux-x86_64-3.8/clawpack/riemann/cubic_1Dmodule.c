...
static PyObject *cubic_1D_error;
static PyObject *cubic_1D_module;
...
#define CHECKSCALAR(check,tcheck,name,show,var)\
    if (!(check)) {\
        ...
        PyErr_SetString(cubic_1D_error,errstring);\
        /*goto capi_fail;*/\
    } else 
#ifdef DEBUGCFUNCS
...
static int int_from_pyobj(int* v,PyObject *obj,const char *errmess) {
    ...
    }
    {
        ...
        if (err==NULL) err = cubic_1D_error;
...
/************************************ rp1 ************************************/
static char doc_f2py_rout_cubic_1D_rp1[] = "\
...
/* extern void F_FUNC(rp1,RP1)(int*,int*,int*,int*,int*,int*,double*,double*,double*,double*,double*,double*,double*,double*); */
static PyObject *f2py_rout_cubic_1D_rp1(const PyObject *capi_self,
                           PyObject *capi_args,
...
/*frompyobj*/
  /* Processing variable maxmx */
    f2py_success = int_from_pyobj(&maxmx,maxmx_capi,"cubic_1D.rp1() 1st argument (maxmx) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mwaves */
    f2py_success = int_from_pyobj(&mwaves,mwaves_capi,"cubic_1D.rp1() 2nd argument (mwaves) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mbc */
    f2py_success = int_from_pyobj(&mbc,mbc_capi,"cubic_1D.rp1() 3rd argument (mbc) can't be converted to int");
  if (f2py_success) {
  /* Processing variable mx */
    f2py_success = int_from_pyobj(&mx,mx_capi,"cubic_1D.rp1() 4th argument (mx) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_ql_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting 5th argument `ql' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_auxl_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting 7th argument `auxl' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_s_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting hidden `s' of cubic_1D.rp1 to C/Fortran array" );
  } else {
    s = (double *)(PyArray_DATA(capi_s_tmp));

  /* Processing variable meqn */
  if (meqn_capi == Py_None) meqn = shape(ql,0); else
    f2py_success = int_from_pyobj(&meqn,meqn_capi,"cubic_1D.rp1() 1st keyword (meqn) can't be converted to int");
  if (f2py_success) {
  ...
  if (maux_capi == Py_None) maux = shape(auxl,0); else
    f2py_success = int_from_pyobj(&maux,maux_capi,"cubic_1D.rp1() 2nd keyword (maux) can't be converted to int");
  if (f2py_success) {
  ...
  if (capi_qr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting 6th argument `qr' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_auxr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting 8th argument `auxr' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_wave_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting hidden `wave' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_amdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting hidden `amdq' of cubic_1D.rp1 to C/Fortran array" );
  } else {
  ...
  if (capi_apdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cubic_1D_error,"failed in converting hidden `apdq' of cubic_1D.rp1 to C/Fortran array" );
  } else {
...
static FortranDataDef f2py_routine_defs[] = {
  {"rp1",-1,{{-1}},0,(char *)F_FUNC(rp1,RP1),(f2py_init_func)f2py_rout_cubic_1D_rp1,doc_f2py_rout_cubic_1D_rp1},
...
#if PY_VERSION_HEX >= 0x03000000
#define RETVAL m
PyMODINIT_FUNC PyInit_cubic_1D(void) {
#else
#define RETVAL
PyMODINIT_FUNC initcubic_1D(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
  m = cubic_1D_module = PyModule_Create(&moduledef);
#else
  m = cubic_1D_module = Py_InitModule("cubic_1D", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module cubic_1D (failed to import numpy)"); return RETVAL;}
...
#endif
    "This module 'cubic_1D' is auto-generated with f2py (version:2).\nFunctions:\n"
"  wave,s,amdq,apdq = rp1(maxmx,mwaves,mbc,mx,ql,qr,auxl,auxr,meqn=shape(ql,0),maux=shape(auxl,0))\n"
".");
  PyDict_SetItemString(d, "__doc__", s);
  Py_DECREF(s);
  cubic_1D_error = PyErr_NewException ("cubic_1D.error", NULL, NULL);
  /*
   * Store the error object inside the dict, so that it could get deallocated.
   * (in practice, this is a module, so it likely will not and cannot.)
   */
  PyDict_SetItemString(d, "_cubic_1D_error", cubic_1D_error);
  Py_DECREF(cubic_1D_error);
...
#ifdef F2PY_REPORT_ATEXIT
  if (! PyErr_Occurred())
    on_exit(f2py_report_on_exit,(void*)"cubic_1D");
#endif
  return RETVAL;
}
#ifdef __cplusplus
