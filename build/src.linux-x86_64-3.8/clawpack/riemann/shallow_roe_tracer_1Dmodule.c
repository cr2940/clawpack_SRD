/* File: shallow_roe_tracer_1Dmodule.c
...
static PyObject *shallow_roe_tracer_1D_error;
static PyObject *shallow_roe_tracer_1D_module;
...
#define CHECKSCALAR(check,tcheck,name,show,var)\
    if (!(check)) {\
        ...
        PyErr_SetString(shallow_roe_tracer_1D_error,errstring);\
        /*goto capi_fail;*/\
    } else 
#ifdef DEBUGCFUNCS
...
static int int_from_pyobj(int* v,PyObject *obj,const char *errmess) {
    ...
    }
    {
        ...
        if (err==NULL) err = shallow_roe_tracer_1D_error;
...
/************************************ rp1 ************************************/
static char doc_f2py_rout_shallow_roe_tracer_1D_rp1[] = "\
...
"num_ghost : input int\n"
"num_cells : input int\n"
"ql : input rank-2 array('d') with bounds (num_eqn,maxmx+num_ghost-(1-num_ghost)+1)\n"
"qr : input rank-2 array('d') with bounds (num_eqn,maxmx+num_ghost-(1-num_ghost)+1)\n"
"auxl : input rank-2 array('d') with bounds (num_aux,maxmx+num_ghost-(1-num_ghost)+1)\n"
"auxr : input rank-2 array('d') with bounds (num_aux,maxmx+num_ghost-(1-num_ghost)+1)\n"
"\nOther Parameters\n----------------\n"
"num_eqn : input int, optional\n    Default: shape(ql,0)\n"
"num_aux : input int, optional\n    Default: shape(auxl,0)\n"
"\nReturns\n-------\n"
"wave : rank-3 array('d') with bounds (num_eqn,num_waves,maxmx+num_ghost-(1-num_ghost)+1)\n"
"s : rank-2 array('d') with bounds (num_waves,maxmx+num_ghost-(1-num_ghost)+1)\n"
"amdq : rank-2 array('d') with bounds (num_eqn,maxmx+num_ghost-(1-num_ghost)+1)\n"
"apdq : rank-2 array('d') with bounds (num_eqn,maxmx+num_ghost-(1-num_ghost)+1)";
/* extern void F_FUNC(rp1,RP1)(int*,int*,int*,int*,int*,int*,double*,double*,double*,double*,double*,double*,double*,double*); */
static PyObject *f2py_rout_shallow_roe_tracer_1D_rp1(const PyObject *capi_self,
                           PyObject *capi_args,
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"maxmx","num_waves","num_ghost","num_cells","ql","qr","auxl","auxr","num_eqn","num_aux",NULL};
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOOOO|OO:shallow_roe_tracer_1D.rp1",\
    capi_kwlist,&maxmx_capi,&num_waves_capi,&num_ghost_capi,&num_cells_capi,&ql_capi,&qr_capi,&auxl_capi,&auxr_capi,&num_eqn_capi,&num_aux_capi))
    return NULL;
/*frompyobj*/
  /* Processing variable maxmx */
    f2py_success = int_from_pyobj(&maxmx,maxmx_capi,"shallow_roe_tracer_1D.rp1() 1st argument (maxmx) can't be converted to int");
  if (f2py_success) {
  /* Processing variable num_waves */
    f2py_success = int_from_pyobj(&num_waves,num_waves_capi,"shallow_roe_tracer_1D.rp1() 2nd argument (num_waves) can't be converted to int");
  if (f2py_success) {
  /* Processing variable num_ghost */
    f2py_success = int_from_pyobj(&num_ghost,num_ghost_capi,"shallow_roe_tracer_1D.rp1() 3rd argument (num_ghost) can't be converted to int");
  if (f2py_success) {
  /* Processing variable num_cells */
    f2py_success = int_from_pyobj(&num_cells,num_cells_capi,"shallow_roe_tracer_1D.rp1() 4th argument (num_cells) can't be converted to int");
  if (f2py_success) {
  /* Processing variable ql */
  ql_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  capi_ql_intent |= F2PY_INTENT_IN;
  capi_ql_tmp = array_from_pyobj(NPY_DOUBLE,ql_Dims,ql_Rank,capi_ql_intent,ql_capi);
  if (capi_ql_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting 5th argument `ql' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    ql = (double *)(PyArray_DATA(capi_ql_tmp));

  /* Processing variable auxl */
  auxl_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  ...
  if (capi_auxl_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting 7th argument `auxl' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    auxl = (double *)(PyArray_DATA(capi_auxl_tmp));

  /* Processing variable s */
  s_Dims[0]=num_waves,s_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  capi_s_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_s_tmp = array_from_pyobj(NPY_DOUBLE,s_Dims,s_Rank,capi_s_intent,Py_None);
  if (capi_s_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting hidden `s' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    s = (double *)(PyArray_DATA(capi_s_tmp));

  /* Processing variable num_eqn */
  if (num_eqn_capi == Py_None) num_eqn = shape(ql,0); else
    f2py_success = int_from_pyobj(&num_eqn,num_eqn_capi,"shallow_roe_tracer_1D.rp1() 1st keyword (num_eqn) can't be converted to int");
  if (f2py_success) {
  ...
  if (num_aux_capi == Py_None) num_aux = shape(auxl,0); else
    f2py_success = int_from_pyobj(&num_aux,num_aux_capi,"shallow_roe_tracer_1D.rp1() 2nd keyword (num_aux) can't be converted to int");
  if (f2py_success) {
  CHECKSCALAR(shape(auxl,0)==num_aux,"shape(auxl,0)==num_aux","2nd keyword num_aux","rp1:num_aux=%d",num_aux) {
  /* Processing variable qr */
  qr_Dims[0]=num_eqn,qr_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  capi_qr_intent |= F2PY_INTENT_IN;
  capi_qr_tmp = array_from_pyobj(NPY_DOUBLE,qr_Dims,qr_Rank,capi_qr_intent,qr_capi);
  if (capi_qr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting 6th argument `qr' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    qr = (double *)(PyArray_DATA(capi_qr_tmp));

  /* Processing variable auxr */
  auxr_Dims[0]=num_aux,auxr_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  ...
  if (capi_auxr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting 8th argument `auxr' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    auxr = (double *)(PyArray_DATA(capi_auxr_tmp));

  /* Processing variable wave */
  wave_Dims[0]=num_eqn,wave_Dims[1]=num_waves,wave_Dims[2]=maxmx+num_ghost-(1-num_ghost)+1;
  ...
  if (capi_wave_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting hidden `wave' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    wave = (double *)(PyArray_DATA(capi_wave_tmp));

  /* Processing variable amdq */
  amdq_Dims[0]=num_eqn,amdq_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  ...
  if (capi_amdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting hidden `amdq' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
    amdq = (double *)(PyArray_DATA(capi_amdq_tmp));

  /* Processing variable apdq */
  apdq_Dims[0]=num_eqn,apdq_Dims[1]=maxmx+num_ghost-(1-num_ghost)+1;
  ...
  if (capi_apdq_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(shallow_roe_tracer_1D_error,"failed in converting hidden `apdq' of shallow_roe_tracer_1D.rp1 to C/Fortran array" );
  } else {
...
#endif
/*callfortranroutine*/
        (*f2py_func)(&maxmx,&num_eqn,&num_waves,&num_aux,&num_ghost,&num_cells,ql,qr,auxl,auxr,wave,s,amdq,apdq);
if (PyErr_Occurred())
  f2py_success = 0;
#ifdef F2PY_REPORT_ATEXIT
...
static FortranDataDef f2py_routine_defs[] = {
  {"rp1",-1,{{-1}},0,(char *)F_FUNC(rp1,RP1),(f2py_init_func)f2py_rout_shallow_roe_tracer_1D_rp1,doc_f2py_rout_shallow_roe_tracer_1D_rp1},
...
#if PY_VERSION_HEX >= 0x03000000
#define RETVAL m
PyMODINIT_FUNC PyInit_shallow_roe_tracer_1D(void) {
#else
#define RETVAL
PyMODINIT_FUNC initshallow_roe_tracer_1D(void) {
#endif
  int i;
  PyObject *m,*d, *s, *tmp;
#if PY_VERSION_HEX >= 0x03000000
  m = shallow_roe_tracer_1D_module = PyModule_Create(&moduledef);
#else
  m = shallow_roe_tracer_1D_module = Py_InitModule("shallow_roe_tracer_1D", f2py_module_methods);
#endif
  Py_TYPE(&PyFortran_Type) = &PyType_Type;
  import_array();
  if (PyErr_Occurred())
    {PyErr_SetString(PyExc_ImportError, "can't initialize module shallow_roe_tracer_1D (failed to import numpy)"); return RETVAL;}
...
#endif
    "This module 'shallow_roe_tracer_1D' is auto-generated with f2py (version:2).\nFunctions:\n"
"  wave,s,amdq,apdq = rp1(maxmx,num_waves,num_ghost,num_cells,ql,qr,auxl,auxr,num_eqn=shape(ql,0),num_aux=shape(auxl,0))\n"
"COMMON blocks:\n""  /cparam/ grav\n"".");
  PyDict_SetItemString(d, "__doc__", s);
  Py_DECREF(s);
  shallow_roe_tracer_1D_error = PyErr_NewException ("shallow_roe_tracer_1D.error", NULL, NULL);
  /*
   * Store the error object inside the dict, so that it could get deallocated.
   * (in practice, this is a module, so it likely will not and cannot.)
   */
  PyDict_SetItemString(d, "_shallow_roe_tracer_1D_error", shallow_roe_tracer_1D_error);
  Py_DECREF(shallow_roe_tracer_1D_error);
...
#ifdef F2PY_REPORT_ATEXIT
  if (! PyErr_Occurred())
    on_exit(f2py_report_on_exit,(void*)"shallow_roe_tracer_1D");
#endif
  return RETVAL;
}
#ifdef __cplusplus
